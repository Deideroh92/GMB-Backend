...
namespace GMB.Tests
{
    ...
    public class UnitTestDaily
    {
        ...
        public async Task BusinessProfileAndScoreTest()
        {
            ...
            (profile, score) = await scannerFunctions.GetBusinessProfileAndScoreFromGooglePageAsync(driver, new(url, null, null), null);
        ...
        {
            List<string>? urls = [];
            List<string> locations = ["paris"];
            ScannerUrlRequest request = new(locations, "banque");
            using SeleniumDriver driver = new(false);
            ScannerFunctions scannerFunctions = new();
            ...
            foreach (string location in locations)
            {
                ...
                List<string>? urlGathered = scannerFunctions.GetUrlsFromGooglePage(driver, url);
                if (urlGathered != null)
